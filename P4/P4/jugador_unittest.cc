...
TEST(Jugador, ConstructorParametros) {
  Jugador j("2", "44XX", "Carlos", "Gutierrez", "C/ Mesa 1", "Aguilar", "Sevilla", "España");
...
TEST(Jugador, setApuestasygetApuestas) {
  ...
  salida.close();
  list<Apuestas> l;
  j.setApuestas(j.getDNI());
  l= j.getApuestas();
  EXPECT_EQ(4, l.size());
  list<Apuestas>::iterator i;
  i=l.begin();
  EXPECT_EQ(1, (*i)._tipo);
  EXPECT_EQ("10", (*i)._valor);
  EXPECT_EQ(15, (*i)._cantidad);
  i++;
  EXPECT_EQ(2, (*i)._tipo);
  EXPECT_EQ("rojo", (*i)._valor);
  EXPECT_EQ(25, (*i)._cantidad);
  i++;
  EXPECT_EQ(3, (*i)._tipo);
  EXPECT_EQ("par", (*i)._valor);
  EXPECT_EQ(35, (*i)._cantidad);
  i++;
  EXPECT_EQ(4, (*i)._tipo);
  EXPECT_EQ("bajo", (*i)._valor);
  EXPECT_EQ(45, (*i)._cantidad);
}
